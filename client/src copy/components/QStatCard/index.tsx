...
const QStatCard: React.FC<Props> = ({ questionData, parentIndex }: Props) => {
  const { updateData } = useQuestionDataContext();
  ...
  async function whenQuestionCompleted(index: number, question: IQuestion) {
    if (question.Done === false) {
      ...
      const updatedQuestions: IQuestion[] = questionData.questions.map(
        (ques: IQuestion, idx: number) => (index === idx ? question : ques)
      );

      const updatedQuestionData: IQuestionData = {
        ...questionData,
        doneQuestions: questionData.doneQuestions + 1,
        started: true,
        questions: updatedQuestions,
        topicName,
      };

      updateData(parentIndex, updatedQuestionData);
    }
  }
                    ...
                    >
                      ...
                      <td className="text-center py-2">
                        <input
                          ...
                          onClick={() => whenQuestionCompleted(index, question)}
                        />
                      </td>
                      {started && (
                        <td className="text-center py-2">
