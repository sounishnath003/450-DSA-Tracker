    ...
    function generateDispatch(index: number) {
        if (index === 0) {
            dispatch({
                ...
                payload: {selectedCategoryType: "easy", selectedCategoryQuestions: easyQuestions}
            });
        } else if (index === 1) {
            dispatch({
                type: "ON_CATEGORY_SELECT",
                payload: {selectedCategoryType: "medium", selectedCategoryQuestions: mediumQuestions}
            });
        } else {
            dispatch({
                type: "ON_CATEGORY_SELECT",
                payload: {selectedCategoryType: "hard", selectedCategoryQuestions: hardQuestions}
            });
        }
    }
    ...
    return <React.Fragment>
        <div className="my-6">
            <div className="flex justify-evenly space-x-4">
                {categories.map((category: CategoryNavbarType, index: number) => <Link key={category.path}
                                                                                       to={category.path}>
                    <div className={`items-center px-6 py-2 border-2 ${
                        index === 0
                            ? `border-green-400 hover:bg-green-200`
                            : index === 1
                            ? "border-blue-400 hover:bg-blue-200"
                            : index === 2 ? `border-red-400 hover:bg-red-200` : `border-yellow-400 hover:bg-yellow-200`
                    } cursor-pointer rounded-lg shadow-lg`}>
                        <div onClick={() => generateDispatch(index)} className="flex space-x-1">
